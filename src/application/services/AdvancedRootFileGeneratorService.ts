...
export class AdvancedRootFileGeneratorService {
  ...
  async generateRootFile(rules: ParsedCursorRule[], rootPath: string): Promise<void> {
  ...
  private buildAdvancedRootContent(rules: ParsedCursorRule[]): string {
    const alwaysApplyRules = rules.filter(r => r.metadata.alwaysApply === true);
    const descriptionRules = rules.filter(r => 
      r.metadata.alwaysApply !== true && r.metadata.description
    );
    const globRules = rules.filter(r => 
      r.metadata.alwaysApply !== true && !r.metadata.description && r.metadata.globs
    );
    ...
    if (alwaysApplyRules.length > 0) {
      content += 'always apply below rules\n';
    ...
    if (descriptionRules.length > 0) {
      content += 'apply below rules if requirement matches\n';
    ...
    if (globRules.length > 0) {
      content += 'apply below rules if glob pattern matches with related files:\n';
